...
void Dibujaranimal(glm::mat4 modelBase, GLuint uniformModel, GLuint uniformColor)
{
	glm::mat4 model;
	glm::mat4 modelaux;
	glm::vec3 color;

	glm::vec3 colorChasis = glm::vec3(0.7f, 0.7f, 0.7f); 
	glm::vec3 colorArticulacion = glm::vec3(0.2f, 0.2f, 0.2f); 
	glm::vec3 colorVisor = glm::vec3(1.0f, 0.9f, 1.0f); 
	...
	color = glm::vec3(0.6f, 0.6f, 0.6f); // Gris claro
	...
	model = glm::scale(model, glm::vec3(1.2f, 1.6f, 1.2f));
	glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
	glUniform3fv(uniformColor, 1, glm::value_ptr(colorChasis));
	meshList[2]->RenderMeshGeometry(); // Cilindro
	...
	model = glm::scale(model, glm::vec3(1.5f, 0.6f, 0.5f));
	glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
	glUniform3fv(uniformColor, 1, glm::value_ptr(colorVisor));
	meshList[0]->RenderMesh(); // Cubo

	glm::vec3 colorOrejaBase = glm::vec3(0.3f, 0.3f, 0.8f); 

	//oreja izquirda
	model = cabezaAux;
	model = glm::translate(model, glm::vec3(1.15f, 0.2f, 0.2f)); 
	model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f)); 
	model = glm::scale(model, glm::vec3(0.3f, 0.25f, 0.3f));
	glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
	glUniform3fv(uniformColor, 1, glm::value_ptr(colorOrejaBase));
	meshList[2]->RenderMeshGeometry(); // Cilindro
	...
	// patas
	float angulosPatas[6] = { 45.0f, -45.0f, 135.0f, 225.0f };
	
	float animaciones[8] = {
		mainWindow.getarticulacion1(), mainWindow.getarticulacion2(), mainWindow.getarticulacion3(),
		mainWindow.getarticulacion4(), mainWindow.getllanta1(), mainWindow.getllanta2(), mainWindow.getllanta3(),
		mainWindow.getllanta4()
	};
	...
	for (int i = 0; i < 4; i++)
	{
		...
		model = glm::rotate(model, glm::radians(angulosPatas[i]), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::translate(model, glm::vec3(1.5f, -0.5f, 0.0f));
		model = glm::rotate(model, glm::radians(animaciones[i]), glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, glm::radians(25.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		glm::mat4 hombroAux = model;
		...
		model = hombroAux;
		model = glm::translate(model, glm::vec3(1.0f, 0.0f, 0.0f)); // Nos movemos la mitad del cilindro
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f)); // Acostamos el cilindro
		...
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
		model = glm::rotate(model, glm::radians(animaciones[i + 4]), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		model = rodillaAux; // Hereda la posición y la rotación animada de la rodilla
		model = glm::translate(model, glm::vec3(1.2f, 0.0f, 0.0f)); 
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f)); 
		model = glm::scale(model, glm::vec3(0.25f, 2.4f, 0.25f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniform3fv(uniformColor, 1, glm::value_ptr(colorChasis));
		meshList[2]->RenderMeshGeometry();
...
void DibujarGrua(glm::mat4 modelBase, GLuint uniformModel, GLuint uniformColor)
{
	glm::mat4 model;
	glm::mat4 modelaux;
	glm::vec3 color;
	...
	glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
	color = glm::vec3(0.5f, 0.9f, 1.0f); // Celeste
	glUniform3fv(uniformColor, 1, glm::value_ptr(color));
	...
	glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
	color = glm::vec3(1.0f, 0.3f, 0.5f); // Rosa
	glUniform3fv(uniformColor, 1, glm::value_ptr(color));
	sp.render();
...
int main()
{
	...
	*/
	...
	camera = Camera(glm::vec3(0.0f, 5.0f, 10.0f), glm::vec3(0.0f, 1.0f, 0.0f), -60.0f, 0.0f, 0.3f, 0.3f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		*/
		...
		posicion = glm::translate(posicion, glm::vec3(0.0f, 0.0f, -4.0f));

		//DibujarGrua(posicion, uniformModel, uniformColor);
		Dibujaranimal(posicion, uniformModel, uniformColor);
