...
int main()
{
	...
	//fuego
	FuegoTexture = Texture("Textures/fuegoA.tga");
	FuegoTexture.LoadTextureA();

	HumoTexture = Texture("Textures/humoA.tga");
	...
	//brazo
	hombro = Model();
	hombro.LoadModel("Models/hombro.obj");
	brazo = Model();
	brazo.LoadModel("Models/brazo.obj");
	esfera_M = Model();
	...
	glm::vec3 posHombro = glm::vec3(1.5f, 4.2f, 2.2f); // La posición que le diste a tu hombro
	glm::vec3 posEsfera;

	// Tiro Parabólico
	glm::vec3 posInicialVuelo;
	glm::vec3 velVuelo;
	float tiempoVuelo = 0.0f;
	const float gravedad = -9.81f;
	const float coeficienteRestitucion = 0.6f;
	...
	while (!mainWindow.getShouldClose())
	{
			...
			// ==========================================
			// 1. LÓGICA DE FUEGO Y HUMO
			// ==========================================
			if (mainWindow.getFuegoEncendido())
			{
				tiempoFuego += deltaTime;
				rotAeolipile += 20.0f * deltaTime; // Gira la turbina
				...
				{
					toffsetFuegoU += 0.25f;
					if (toffsetFuegoU >= 1.0f) toffsetFuegoU = 0.0f;
					framesFuego = 0;
				}


				if (escalaHumo < 4.0f) {
					escalaHumo += 1.0f * deltaTime;
				}

				if (escalaHumo >= 4.0f && !esferaLanzada && rebotes == 0) {
					brazoLanzando = true;
				}


				if (tiempoFuego >= 100.0f)
				{
					mainWindow.setFuegoEstado(false);
					tiempoFuego = 0.0f;
					toffsetFuegoU = 0.0f;
				}
			}
			else
			{
				...
				if (escalaHumo > 0.0f) {
					escalaHumo -= 1.0f * deltaTime;
					if (escalaHumo < 0.0f) escalaHumo = 0.0f;
				}
			}
			...
			if (escalaHumo > 0.0f) {
				...
				if (framesHumo >= 100) {
					toffsetHumoU += 0.25f;
					if (toffsetHumoU >= 1.0f) toffsetHumoU = 0.0f;
					framesHumo = 0;
				}
			}
			...
			if (brazoLanzando) {
				...
				if (rotBrazo <= -60.0f) {
					...
					float anguloRad = rotBrazo * toRadians;
					posInicialVuelo = posHombro + glm::vec3(2.0f * cos(anguloRad), 2.0f * sin(anguloRad), 0.0f);

					velVuelo = glm::vec3(0.0f, 4.0f, -3.0f);
				}
			}
			...
			if (esferaLanzada) {
				...
				tiempoVuelo += 0.02f * deltaTime;

				posEsfera.x = posInicialVuelo.x + (velVuelo.x * tiempoVuelo);
				posEsfera.z = posInicialVuelo.z + (velVuelo.z * tiempoVuelo);
				posEsfera.y = posInicialVuelo.y + (velVuelo.y * tiempoVuelo) + (0.5f * gravedad * tiempoVuelo * tiempoVuelo);

				float piso_Y = 0.25f; 
				if (posEsfera.y <= piso_Y) {
					posEsfera.y = piso_Y; // Choca

					float velImpactoY = velVuelo.y + (gravedad * tiempoVuelo);
					velVuelo.y = -velImpactoY * coeficienteRestitucion; // Rebota

					velVuelo.x *= 0.8f; 
					velVuelo.z *= 0.8f;

					posInicialVuelo = posEsfera;
					tiempoVuelo = 0.0f;
					rebotes++;
				}
			...
			glm::mat4 matrizAeolipileBase = model;
		...
		//fuego 
			...
			if (escalaHumo > 0.0f) {
				toffset = glm::vec2(toffsetHumoU, 0.0f);
				glUniform2fv(uniformTextureOffset, 1, glm::value_ptr(toffset));
			...
			//brazoo
			model = glm::mat4(1.0);
			model = glm::translate(model, glm::vec3(1.5f, 4.2f, 2.2f));
			glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
			hombro.RenderModel();
